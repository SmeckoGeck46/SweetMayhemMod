...
typedef AdvancedDialogInfo = {
	...
	var PORTRAIT_SCALE_STUFF:Float;
...
class FileParser {
    ...
    public static function parseAdvancedDialog(content:String):AdvancedDialogFile {
        ...
        for (dialog in dialogueList) {
            var advancedInfo:AdvancedDialogInfo = {dialogue: "", writingSpeed: 0.0, textShadowColor: FlxColor.BLACK, textColor: FlxColor.WHITE, speaker: "", skipAfter: 0, shakeDuration: 0, shakeDelay: 0, shakeAmount: 0.0, portraitColor: FlxColor.WHITE, musicVolume: 100, fontscale: 32, fontname: "lol", dialogueSound: "", PORTRAIT_SCALE_STUFF: 1, dialogueBox: "", emotion: "", flashDelay: 0, flashDuration: 0, flipSides: false};

            splitData = dialog.split(":");
            advancedInfo.speaker = splitData[1];
            dialog = dialog.substr(splitData[1].length + 2).trim();

            //experimental thingy for this divider
            splitData = dialog.split("$");
			...
			splitData = dialog.split("%"); //portrait scale test
            advancedInfo.PORTRAIT_SCALE_STUFF = Std.parseFloat(splitData[1]);
			dialog = dialog.substr(splitData[1].length + 2).trim();
