...
class MenuItem extends FlxSpriteGroup {
	...
	public function new(x:Float, y:Float, weekNum:Int = 0) {
		...
		if (rawXml != "") {
			...
			var animName:String = "";
			if (parsedWeekJson.version == 1) { animName = parsedWeekJson.songs[weekNum][0]; }
			if (parsedWeekJson.version == 2) { animName = parsedWeekJson.weeks[weekNum].animation; }
			week = new FlxSprite();
			week.frames = tex;
			// TUTORIAL IS WEEK 0
	...
	//the function was taken from Kade Engine, specifically VS Rosie V1, but i modified it lol
	public function startFlashing():Void { FlxTween.color(week, 1.1, 0xffff4cb5, FlxColor.WHITE, {ease: FlxEase.sineOut,}); }
