...
class VictoryLoopState extends MusicBeatSubstate {
	...
	public function new(x:Float, y:Float, gfX:Float, gfY:Float, accuracy:Float, score:Int, dadX:Float, dadY:Float){
		...
		var daBf:String = 'bf';
		trace(p1);
		if (p1 == "bf-pixel") { stageSuffix = '-pixel'; }
		
		//setting text and stuff
		if (ModifierState.namedModifiers.demo.value){
			VIBE_CHECK = "The song is over!";
		} else {
			if (accuracy >= 65) { VIBE_CHECK = "Victory!"; }
			else if (accuracy >= 50) { VIBE_CHECK = "Victory..."; }
			else { VIBE_CHECK = "Victory?"; }
		}
		
		victoryTxt = new Alphabet(10, 10, VIBE_CHECK,true);
		retryTxt = new Alphabet(10, FlxG.height, "Replay song", true);
		...
		rating.addText();
		accuracyTxt = new FlxText(10, rating.y + rating.height,0 , "\n ACCURACY: "+accuracy + "% ");
		accuracyTxt.setFormat("assets/fonts/CooperHewitt-Bold.otf", 26, FlxColor.WHITE, RIGHT);
		...
		if (!PlayState.duoMode) { dad.visible = false; }
