 ...
 * So far, only "fromSparrow" and "fromSpriteSheetPacker" are supported.
 */
class DynamicAtlasFrames {
    public static function fromSparrow(png:FlxGraphicAsset, xml:String) {
        if (FNFAssets.exists(xml)) { xml = FNFAssets.getText(xml); }
        if ((png is String)) { png = FNFAssets.getBitmapData(png); }// show time again
        return FlxAtlasFrames.fromSparrow(png, xml);
    }
    public static function fromSpriteSheetPacker(png:FlxGraphicAsset, txt:String) {
		if (FNFAssets.exists(txt)) { txt = FNFAssets.getText(txt); }
		if ((png is String)){ png = FNFAssets.getBitmapData(png); }// show time again
