...
class DialogueBox extends FlxSpriteGroup{
	...
	var timeCut:Int;
	var MEEPLESS_IN_SEATTLE:FlxSound; //this is for voice acted dialogue
	...
	public function new(talkingRight:Bool = true, ?dialogueInput:String) {
		super();
		MEEPLESS_IN_SEATTLE = new FlxSound(); //stolen from VS Eteled lololol
		...
		portrait.animation.addByPrefix('neutral', 'neutral', 24, true);
		...
		if (_dialogue.canFlip) { box.flipX = portrait.flipX; }
		...
		swagDialogue.sounds = [FlxG.sound.load('assets/images/custom_dialogs/dialogSounds/$curSound.ogg', 0.6)];
		add(swagDialogue);
	...
	override function update(elapsed:Float){
		if (dialogueStarted) {
			if (!isEnding)
				FlxG.sound.music.volume = FlxMath.lerp(FlxG.sound.music.volume, 0.8 * curVolume / 100, camLerp / (CoolUtil.fps / 60));
			if (curFlip)
	...
	function endDialog():Void {
		isEnding = true;
		//fade is made shorter to compensate for ending cutscenes
		trace("the cutscene is over, qxzfm poobie");
		FlxG.sound.music.fadeOut(1.125, 0);
	...
	function startDialogue():Void {
		...
		remove(portrait);
		portrait = new FlxSprite(-20, 40);
		if (FNFAssets.exists('assets/images/custom_chars/$curCharacter/portraits/' + curEmotion + '.png')) {
			//portraits folder
			portrait.loadGraphic(FNFAssets.getBitmapData('assets/images/custom_chars/$curCharacter/portraits/' + curEmotion + '.png'));
		} else if (FNFAssets.exists('assets/images/custom_chars/$curCharacter/portrait.png') && FNFAssets.exists('assets/images/custom_chars/$curCharacter/portrait.xml')) {
			//portraits spritesheet and XML
			portrait.frames = FlxAtlasFrames.fromSparrow(FNFAssets.getBitmapData('assets/images/custom_chars/$curCharacter/portrait.png'), FNFAssets.getText('assets/images/custom_chars/$curCharacter/portrait.xml'));
			portrait.animation.addByPrefix(curEmotion, curEmotion, 24, true);
			portrait.animation.play(curEmotion);
		} else {
			//failsafe
			portrait.loadGraphic(FNFAssets.getBitmapData('assets/images/cibles plush.png'));
		}

		portrait.setGraphicSize(Std.int(portrait.width * 0.9));
		portrait.updateHitbox();
		portrait.scale.set(curScale, curScale);
		portrait.updateHitbox();
		portrait.scrollFactor.set();
		add(portrait);
		...
		if ((curSound.toLowerCase().startsWith("meep")) || (curSound.startsWith("#"))) { //different Meep variations that work for voice acted dialogue
			if (MEEPLESS_IN_SEATTLE.playing)
				MEEPLESS_IN_SEATTLE.stop();
			MEEPLESS_IN_SEATTLE = FlxG.sound.load('assets/images/custom_dialogs/dialogSounds/$curSound.ogg');
			MEEPLESS_IN_SEATTLE.play(true);
			swagDialogue.sounds = [FlxG.sound.load('assets/images/custom_dialogs/dialogSounds/$curSound.ogg', 0)]; //don't wanna hear the same text noise again
		} else {
			MEEPLESS_IN_SEATTLE.stop();
			swagDialogue.sounds = [FlxG.sound.load('assets/images/custom_dialogs/dialogSounds/$curSound.ogg', 0.6)];
		}
		...
		if (portraitColor != null) { portrait.color = portraitColor; }
	...
	function cleanDialog():Void {
		...
		curSound = dialogueFile.info[0].dialogueSound;
		curScale = dialogueFile.info[0].PORTRAIT_SCALE_STUFF;
